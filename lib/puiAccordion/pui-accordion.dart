...
class PuiAccordionComponent extends NgShadowRootAware {
  ...
  @NgAttr("toggleable")
  set toggleable(String s) {_toggleable=s; addOrRemoveMouseListeners();}
  ...
  String headerIconClass() => collapsedState? "ui-icon-triangle-1-e" : "ui-icon-triangle-1-s";
  ...
  addOrRemoveMouseListeners()
  {
    if (titlebar!=null)
    {
      if (_toggleable==null || _toggleable == "true") {
        ...
        listenOnMouseEnter = titlebar.onMouseEnter.listen(
            (event) => titlebar.classes.add('ui-state-hover'));
        listenOnMouseLeave=titlebar.onMouseLeave.listen(
            (event) => titlebar.classes.remove('ui-state-hover'));
        listenOnClick=titlebar.onClick.listen(
            (event) => toggle(event));
      }
      else
      {
